def official(do_print):
    import numpy as np

    HW = [bin(n).count("1") for n in range(0, 256)]

    sbox = (
        )


    def intermediate(pt, keyguess):
        return sbox[pt ^ keyguess]


    dir = '/home/cwuser/chipwhisperer/projects/tmp/default_data/traces/'
    power_traces = np.load(dir + '2016.07.27-10.37.34_traces.npy')
    textin = np.load(dir + '2016.07.27-10.37.34_textin.npy')
    correct_key = np.load(dir + '2016.07.27-10.37.34_knownkey.npy')

    num_power_traces = np.shape(power_traces)[0] - 1
    num_trace_readings = np.shape(power_traces)[1]

    # Use less than the maximum traces by setting numtraces to something
    # numtraces = 15

    out = open('official.out', 'w')
    bestguess = [0] * 16
    pge = [256]*16

    # Set 16 to something lower (like 1) to only go through a single subkey & save time!
    for subkey in range(0, 16):
        cpaoutput = [0] * 256
        maxcpa = [0] * 256
        for keyguess in range(0, 256):
            #print "Subkey %2d, hyp = %02x: " % (subkey, keyguess),
            #out.write("Subkey %2d, hyp = %02x: " % (subkey, keyguess))
            # Initialize arrays & variables to zero
            sumnum = np.zeros(num_trace_readings)
            sumden1 = np.zeros(num_trace_readings)
            sumden2 = np.zeros(num_trace_readings)

            hyp = np.zeros(num_power_traces)
            for trace in range(0, num_power_traces):
                hyp[trace] = HW[intermediate(textin[trace][subkey], keyguess)]

            # Mean of hypothesis
            mean_weight = np.mean(hyp, dtype=np.float64)

            # Mean of all points in trace
            mean_power = np.mean(power_traces, axis=0, dtype=np.float64)

            # For each trace, do the following
            for trace in range(0, num_power_traces):
                hdiff = (hyp[trace] - mean_weight)
                tdiff = power_traces[trace, :] - mean_power

                sumnum = sumnum + (hdiff * tdiff)
                sumden1 = sumden1 + hdiff * hdiff
                sumden2 = sumden2 + tdiff * tdiff

            cpaoutput[keyguess] = sumnum / np.sqrt(sumden1 * sumden2)
            maxcpa[keyguess] = max(abs(cpaoutput[keyguess]))

            #print maxcpa[keyguess]
            #out.write(str(maxcpa[keyguess]) + '\n')

        # Find maximum value of key
        bestguess[subkey] = np.argmax(maxcpa)
        #cparefs = np.argsort(maxcpa)[::-1]

        #pge[subkey] = list(cparefs).index(correct_key[subkey])

    if do_print:
        print "Best Key Guess: "
        out.write("Best Key Guess: ")
        for b in bestguess:
            print "%02x " % b,
            out.write("%02x " % b)

        print ''
        for s in correct_key:
            print "%02x " % s,

        '''
        "print ""
        print "PGE: ",
        for b in pge:
            print "%02d "%b,
        '''